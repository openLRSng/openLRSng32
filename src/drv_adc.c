...
void adcInit(drv_adc_config_t *init)
{
    ...
    uint8_t multiChannel = ((init->analogEnable >= 3) ? 1 : 0);
    ...
    if (init->analogEnable && 0x01) {
        ADC_RegularChannelConfig(ADC1, adcDefine[0].channel, 1, ADC_SampleTime_28Cycles5);
    }
    if (init->analogEnable && 0x02) {
        ADC_RegularChannelConfig(ADC1, adcDefine[1].channel, 1, ADC_SampleTime_28Cycles5);
    }
